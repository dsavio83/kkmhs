...
const MarkInputRow: React.FC<MarkInputRowProps> = ({ 
  student, teMark, ceMark, attendance, teEnabled, ceEnabled, maxTeMarks, maxCeMarks, onUpdate 
}) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'te' | 'ce' | 'att') => {
    ...
    // Auto-jump logic (only for marks)
    if (type !== 'att') {
        ...
        if (shouldJump) {
        if (type === 'te' && ceEnabled && ceRef.current) {
            ceRef.current.focus();
            setTimeout(() => ceRef.current?.select(), 10);
        } else {
      ...
      <td className="px-6 py-3 align-top text-center">
         <div className="relative inline-block">
            <input 
              ...
              className="w-16 px-2 py-2.5 border border-slate-200 bg-purple-50/50 text-purple-700 rounded-xl text-center font-bold outline-none focus:ring-2 focus:ring-purple-200 transition-all shadow-sm"
              placeholder="%"
            />
         </div>
      </td>
        ...
        <td className="px-6 py-3 align-top text-center">
          <div className="relative inline-block">
            <input
              ...
              className={`w-24 px-3 py-2.5 border ${isTeInvalid ? 'border-red-500 bg-red-50 text-red-600 focus:ring-red-200' : getInputStyle(teMark, maxTeMarks)} rounded-xl text-center font-bold outline-none transition-all shadow-sm`}
        ...
        <td className="px-6 py-3 align-top text-center">
          <div className="relative inline-block">
            <input
              ...
              className={`w-24 px-3 py-2.5 border ${isCeInvalid ? 'border-red-500 bg-red-50 text-red-600 focus:ring-red-200' : getInputStyle(ceMark, maxCeMarks)} rounded-xl text-center font-bold outline-none transition-all shadow-sm`}
