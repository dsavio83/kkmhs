...
const ExamManager: React.FC<ExamManagerProps> = ({ teacher, state, setState }) => {
  ...
  const handleOpenModal = (exam?: Exam) => {
    if (exam) {
      ...
      setConfigs(exam.subjectConfigs);
    } else {
      setEditingExam(null);
      setExamName('');
      setSelectedClassId('');
    ...
    try {
        ...
        setState((prev: any) => {
            const updatedExams = editingExam 
                ? prev.exams.map((ex: any) => ex.id === editingExam.id ? newExam : ex)
                : [...prev.exams, newExam];
  ...
  const myExams = state.exams.filter((ex: any) => myFormClasses.some((c: any) => c.id === ex.classId));
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {myExams.length > 0 ? myExams.map((exam: Exam) => {
          const cls = state.classes.find((c: any) => c.id === exam.classId);
          return (
               ...
               <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exam.subjectConfigs.map((c: any) => {
                        const sub = state.subjects.find((s: any) => s.id === c.subjectId);
                        return (
                            <span key={c.subjectId} className="px-2 py-1 bg-white border border-slate-200 rounded text-[10px] font-bold text-slate-600">
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {configs.map((config) => {
                            ...
                            return (
                                <div key={config.subjectId} className={`flex items-center gap-4 p-3 rounded-xl border ${config.included ? 'bg-blue-50/50 border-blue-100' : 'bg-slate-50 border-slate-100 opacity-60'}`}>
                                    <button 
                                      type="button" 
                                    ...
                                    <span className="flex-1 font-bold text-sm text-slate-700">{subject.name}</span>
                                    
                                    {config.included && (
                                        <div className="flex items-center gap-2">
                                            <div className="flex flex-col items-center">
                                                <span className="text-[8px] font-black uppercase text-slate-400 mb-0.5">Max TE</span>
                                                <input 
                                                  type="number"
                                                  value={config.maxTe}
                                                  onChange={(e) => updateConfig(config.subjectId, 'maxTe', parseInt(e.target.value) || 0)}
                                                  className="w-16 bg-white border border-slate-200 rounded-lg py-1 text-center font-bold text-sm"
                                                />
                                            </div>
                                            <div className="flex flex-col items-center">
                                                <span className="text-[8px] font-black uppercase text-slate-400 mb-0.5">Max CE</span>
                                                <input 
                                                  type="number"
                                                  value={config.maxCe}
